...
const Payment = ({ onClose, initialCoins = 10 }) => {
  ...
  return (
    <div className="bg-[#1E1E1E] border border-[#1FFFE0]/30 rounded-xl p-6 mx-auto max-w-md w-full relative">
  {/* Close Button */}
  <button  
    onClick={onClose}
    disabled={loading}
    className="absolute top-3 right-3 text-gray-400 hover:text-[#1FFFE0] transition-colors disabled:opacity-50"
  >
    <X size={24} />
  </button>
  ...
  <div className="flex flex-col items-center mb-6">
    ...
    <h2 className="text-2xl font-bold text-[#1FFFE0] text-center">Purchase LinkCoins</h2>
  ...
  {/* LinkCoins Selection */}
  <div className="mb-6">
    <label className="block text-[#1FFFE0] text-sm font-medium mb-2">Adjust Quantity</label>
    <div className="flex items-center justify-center">
      <button
        onClick={() => setLinkCoins((prev) => Math.max(1, prev - 1))}
        disabled={loading}
        className="px-4 py-3 bg-[#252525] text-[#1FFFE0] rounded-l-lg hover:bg-[#1FFFE0]/10 transition-colors disabled:opacity-50"
      >
        -
      </button>
      <div className="px-2 py-3 bg-[#252525] text-[#1FFFE0] font-bold text-xl flex items-center gap-2">
        <img 
          src="/coin.png" 
          alt="LinkCoins" 
      ...
      <button
        ...
        disabled={loading}
        className="px-4 py-3 bg-[#252525] text-[#1FFFE0] rounded-r-lg hover:bg-[#1FFFE0]/10 transition-colors disabled:opacity-50"
      >
        +
      </button>
    </div>
    ...
    <div className="flex justify-between items-center">
      ...
      <div className="flex items-center gap-2">
        <img 
          ...
          className="w-8 h-8 object-contain" 
        />
        <span className="text-2xl font-bold text-[#1FFFE0]">₹{totalAmount}</span>
      </div>
    </div>
  </div>
